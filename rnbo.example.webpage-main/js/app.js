async function setup() {
    const patchExportURL = "export/prova_giulione.export.json";
...
// Chiamato dopo che RNBO ha creato il device
function enableHandTracking(device) {

    // Map dei parametri
    const paramMap = {
        4: device.parametersById.get("cutoff"),        
        8: device.parametersById.get("pulsewidth1"),        
        12: device.parametersById.get("pulsewidth2"),
        16: device.parametersById.get("gigaverb/spread"),
        20: device.parametersById.get("res")
    };
    ...
    function resizeCanvas() {
        ...
        console.log("Canvas reso a", canvasElement.width, "x", canvasElement.height);
    }
    ...
    hands.onResults(results => {
        ...
        if (results.multiHandLandmarks?.length > 0) {
            ...
            Object.entries(paramMap).forEach(([id, param]) => {
                if (!param) return;

                const landmark = hand[id];

                const normalized = 1 - landmark.y;
                const scaledValue =
                    normalized * (param.max - param.min) + param.min;
                param.value = scaledValue;
                ...
                canvasCtx.fillText(
                    ...
                    landmark.y * canvasElement.height - 10
                );
            });
        }
